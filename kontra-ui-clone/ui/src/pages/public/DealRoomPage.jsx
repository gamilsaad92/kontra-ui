...
function ReadinessPanel({ property }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-5">
      <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-4">Transaction Review</p>
      <div className="flex items-center gap-3 mb-4">
        <div className="text-2xl font-black" style={{ color: done >= 4 ? "#16a34a" : "#d97706" }}>{done}/5</div>
        <div>
          <p className="text-sm font-bold text-gray-900">Review Areas</p>
          <div className="mt-1 h-2 w-32 rounded-full bg-gray-100 overflow-hidden">
      ...
      {pillars.map((p) => (
        <div key={p.label} className="flex items-center gap-2.5 py-1.5 border-t border-gray-100 first:border-t-0">
          ...
          <span className="text-xs font-semibold" style={{ color: p.done ? "#16a34a" : "#d97706" }}>{p.done ? "Reviewed ✓" : "Pending"}</span>
        </div>
      ))}
    </div>
  );
...
// ── Transaction review summary bar (demo rooms) ──────────────────────────────
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-6">
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">Transaction Review</p>
          <p className="text-base font-bold text-gray-900 mt-0.5">
            <span style={{ color }}>{done}/5 review areas complete</span>
            <span className="text-sm font-medium text-gray-400 ml-2">
              {done >= 5 ? "— Review summary complete ✓" : done >= 4 ? "— Near complete" : "— In progress"}
            </span>
          </p>
        </div>
...
function buildPendingSectionMap(property, role, onAnalysisSaved, urlPropertyId, refreshKey, pack) {
  ...
  return {
    ...
    readiness:  () => <PendingPanel title="Transaction Review" icon="🏅" description="Review areas will be tracked as parties submit their documentation." />,
...
function AssetReadinessTab({ propertyId, property, pack, onTabChange }) {
  ...
  return (
    <div className="space-y-5">
            ...
            <div key={p.id}
              ...
              <div className="min-w-0">
                ...
                {p.active
                  ...
                  : <p className="text-[9px] text-gray-400">Not connected</p>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex items-center gap-2 mb-1">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">Digital Asset Readiness Export</p>
          <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-gray-100 text-gray-400 uppercase">
            Not connected
          </span>
        </div>
        <p className="text-[11px] text-gray-400 mb-4 leading-snug">
           The structured transaction record can be exported for review by an
          external digital-asset provider. Kontra prepares and exports — it never
          issues tokens, provides custody, or executes a transaction.
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
          {[
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex items-center gap-2 mb-1">
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">External Compliance Review</p>
          <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-gray-100 text-gray-400 uppercase">
            Not connected
          </span>
        </div>
        <p className="text-[11px] text-gray-400 mb-4 leading-snug">
           KYC, AML, transfer-agent, and custody work remains with qualified external parties.
           Kontra can coordinate documents and review status; it does not perform the regulated activity.
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
          {[
        ...
        <div className="flex items-start justify-between gap-4 border-b border-gray-200 bg-white px-5 py-4 sm:px-7">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#800020]">
              {isDemo ? 'Illustrative completed snapshot' : 'Immutable Verified Asset record'}
            </p>
              ...
              packageHistory.find(item => item.source_snapshot_id === snapshot.id)
                ? (
                  ...
                  <button
                    ...
                    disabled={isDemo || !ownerToken || packageAction?.loading}
            ...
            <div className="mt-5 rounded-xl border border-gray-200 bg-white px-4 py-4">
              ...
              {isDemo ? (
                <p className="mt-2 text-[11px] font-semibold text-indigo-700">
                  This illustrative completed snapshot is read-only in the public demo.
                </p>
...
function DigitalAssetPackageModal({
  ...
  useEffect(() => {
    let cancelled = false;
    if (readOnly || !packageRecord?.id || !propertyId) return undefined;
    setRevisionLoadError('');
    ...
    return () => { cancelled = true; };
  }, [propertyId, packageRecord?.id, packageRecord?.revision, pdfMetadataRefreshKey, readOnly]);
  ...
  function renderPreparationInput(key, field) {
    ...
    const disabled = readOnly || !ownerToken || saveState.loading;
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-gray-200 bg-white px-5 py-4 sm:px-7">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#800020]">
               {readOnly ? 'Illustrative completed preparation package' : 'Snapshot-bound preparation artifact'}
            </p>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
             ...
             <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-4">
                <p className="text-[10px] font-bold uppercase tracking-wider text-amber-800">
                  {readOnly ? 'Preparation inputs (read-only exhibit)' : 'Editable preparation inputs'}
                </p>
               <p className="mt-1 text-xs text-gray-500">
                  {readOnly
                    ? 'This illustrative package is provided for inspection only. Demo visitors cannot edit it or create revisions.'
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 pt-4">
        <div className="min-w-0 flex-1">
          <p className="text-xs text-gray-500">
            {isDemo
              ? 'Public demo exhibit — no room changes are available.'
              : ownerToken
        ...
        <button
          ...
          onClick={onRecordSnapshot}
           disabled={isDemo || !ownerToken || action.loading}
        ...
        >
           {action.loading ? 'Recording…' : isDemo ? 'Demo snapshot is read-only' : 'Record readiness snapshot'}
        </button>
      </div>
...
function CoordinatorOverview({ propertyId, property, pack, packId, onTabChange, refreshKey, onDocumentRequest, isDemo = false }) {
...
// Settlement modes are recorded coordination choices, not Kontra provider
// integrations. Kontra never holds funds or executes the transfer.
const SETTLEMENT_PROVIDERS = [
  {
    ...
    icon:   '🏦',
    active: true,
    desc:   'Record bank-to-bank settlement instructions for coordination with the relevant parties',
    note:   'Add non-sensitive instructions in the notes below. Your attorney, escrow officer, or other authorized party coordinates the actual transfer.',
  },
  {
    id:     'escrow',
    label:  'Escrow',
    icon:   '🔐',
    active: true,
    desc:   'Record a third-party holdback arrangement and its release conditions',
    note:   'Name the escrow party and release conditions. Kontra records this in the deal-room history; it does not hold or release funds.',
  },
  {
    id:     'stablecoin',
    label:  'Stablecoin',
    icon:   '💵',
    active: false,
    desc:   'Digital-asset settlement would require a separately reviewed external provider',
    note:   '',
  },
  {
    id:     'stripe',
    ...
    icon:   '⚡',
    active: false,
    desc:   'Payment processing is not connected to this deal room',
    note:   '',
  },
  {
    id:     'cbdc',
    ...
    icon:   '🏛️',
    active: false,
    desc:   'No central-bank digital currency connection is available',
    note:   '',
  },
  {
    id:     'ramp_bridge',
    ...
    icon:   '🔄',
    active: false,
    desc:   'No digital-asset provider connection is available',
    note:   '',
  },
];
    ...
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      ...
      <div className="px-6 py-5">
            ...
            return (
              <button
                ...
                <div className="flex items-center justify-between w-full">
                  <span className="text-base">{p.icon}</span>
                  {!p.active && (
                    <span className="text-[8px] font-bold text-gray-400 uppercase tracking-wider">Not connected</span>
                  )}
                  {isSelected && p.active && (
        ...
        <p className="text-[9px] text-gray-300 mt-4 leading-snug">
          Kontra does not hold or process funds. Settlement is handled by the relevant external party.
          No payment, custody, blockchain, or digital-asset provider connection is active here.
        </p>
      </div>
    </div>
  );
...
function OperationsManagerView({ propertyId, property, pack, role, onTabChange }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Jurisdiction review card (digital-asset preparation only) ─── */}
      {/* The card organizes jurisdiction-specific questions for professional
          review; it does not provide legal or regulatory guidance. */}
      ...
      {/* A scored, categorized view of preparation information. Suggested
          preparation only — not a legal or regulatory determination. */}
      {false && isTokenization && (() => {
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
        ...
        {/* ── Participant document progress (digital-asset preparation only) ─ */}
        {false && isTokenization && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Participant Document Progress</p>
            ...
            {kycTotal != null && kycTotal > 0 ? (
              <>
                ...
                <div className="flex items-center gap-4">
                  <span className="text-xs text-gray-500">
                    <span className="font-bold text-gray-800">{kycVerified}</span> reviewed
                  </span>
                  {kycPending > 0 && (
                    <span className="text-xs text-amber-600 font-semibold">
                      {kycPending} pending participant review
                    </span>
                  )}
                </div>
              </>
              ...
              <p className="text-xs text-gray-400">
                Participant document progress will appear after the configured supporting documents are uploaded and analyzed.
              </p>
            )}
          </div>
        )}
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-3xl p-10 max-w-md w-full text-center shadow-2xl">
          ...
          <ul className="text-left space-y-3 mb-8">
            {[
              ...
              "AI-assisted review can surface document flags",
       ...
       <div className="relative z-10 isolate pointer-events-auto max-w-5xl mx-auto px-6 py-8">
        ...
        {isCurrentWorkspace ? (
          ...
          <>
            ...
            {/* Transaction review summary bar — demo rooms only */}
            {!property.isCustom && (
              ...
              <div className="rounded-2xl overflow-hidden border border-indigo-100 mt-2">
                <div className="px-8 py-8 text-center" style={{ background: "linear-gradient(135deg, #1e1b4b 0%, #4338ca 100%)" }}>
                  ...
                  <p className="text-sm text-white/60 mb-6 max-w-md mx-auto">
                    Set up a deal room for your property in minutes. AI-assisted review can help surface document flags, and each party gets a role-scoped view.
                  </p>
                  ...
                  <p className="text-[10px] text-white/30 mt-4">One-time fee · No subscription · Access during the applicable active period</p>
                </div>
