...
export default function PortfolioOverviewPage() {
  ...
  const kpis = useMemo(() => {
    const amounts = loans
      .map((l) => getLoanAmount(l.data as Record<string, unknown>))
      .filter((n): n is number => n !== undefined);
    const ltvs = loans
      .map((l) => getLtv(l.data as Record<string, unknown>))
      .filter((n): n is number => n !== undefined);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <KpiCard
          label="Total AUM"
          value={kpis.aum > 0 ? formatUSD(kpis.aum) : '—'}
          sub={`${kpis.total} loan${kpis.total !== 1 ? 's' : ''}`}
        />
        <KpiCard
          label="Avg LTV"
          value={kpis.avgLtv !== undefined ? `${kpis.avgLtv.toFixed(1)}%` : '—'}
          sub="across portfolio"
        />
        <KpiCard
          label="Watch / Delinquent"
          value={`${kpis.watchCount + kpis.delinqCount}`}
          sub="require attention"
          accent={kpis.watchCount + kpis.delinqCount > 0}
        />
        <KpiCard
          label="Maturing ≤ 90d"
          value={`${kpis.maturingSoon}`}
          sub="loans"
          accent={kpis.maturingSoon > 0}
        />
      </div>
