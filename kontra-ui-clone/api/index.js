...
// manipulated to point at another room's object.
app.get(['/api/public/document-url', '/api/public/deal-room/:propertyId/document/:documentId/url'], async (req, res) => {
  const propertyId = String(req.params.propertyId || req.query.propertyId || '').trim();
  const documentId = String(req.params.documentId || req.query.documentId || req.query.analysisId || '').trim();
  ...
  try {
    ...
    if (access.mode === 'participant') {
      ...
      if (!assignedSections.has(document.section)) {
    ...
    logEvent(
      ...
      access.role,
      access.email || null,
      `${document.filename || 'Document'} original accessed`,
...
;(() => {
  ...
  app.get(`/api/public/deal-room/${DEMO_ID}/verified-asset/readiness`, (_req, res) =>
    res.json(fixture.verifiedAssetExhibit.readiness));
  app.get(`/api/public/deal-room/${DEMO_ID}/verified-asset/snapshots`, (_req, res) =>
    res.json({ snapshots: [fixture.verifiedAssetExhibit.snapshot] }));
  app.get(`/api/public/deal-room/${DEMO_ID}/verified-asset/snapshots/1`, (_req, res) =>
    res.json({ snapshot: fixture.verifiedAssetExhibit.snapshot }));
  app.get(`/api/public/deal-room/${DEMO_ID}/digital-asset-packages`, (_req, res) =>
    res.json({ packages: [fixture.verifiedAssetExhibit.package] }));
  app.get(`/api/public/deal-room/${DEMO_ID}/digital-asset-packages/${fixture.verifiedAssetExhibit.package.id}`, (_req, res) =>
...
async function syncGeneratedProposalToTransactionRecord(propertyId, proposal, actorEmail = 'Deal Owner') {
  ...
  for (const field of proposalFields) {
    ...
    const key = canonicalizeTransactionRecordKey(
      semanticRecordKey(String(field.key), field.label || field.display_label || '') || String(field.key),
      'generic',
    );
...
async function upsertTransactionRecordConflict({
  ...
  const semantic = inferSemanticDefinition(fieldKey, null, displayLabel || '');
  const comparison = compareComparableValues(
    normalizeSemanticComparableValue(canonicalValue, semantic),
    normalizeSemanticComparableValue(conflictingValue, semantic),
    semantic,
  );
  ...
  try {
    ...
    const normalizeComparableValue = (value, fieldKey, displayLabel) =>
      normalizeSemanticComparableValue(
        value,
        inferSemanticDefinition(fieldKey, value, displayLabel),
        displayLabel,
      );
    const semanticFieldKey = (rawKey, displayLabel) => {
      return semanticRecordKey(rawKey, displayLabel);
    };
...
app.post('/api/public/deal-room/:propertyId/transaction-record/conflicts/:conflictId/resolve', async (req, res) => {
  ...
  try {
    ...
    // A legacy conflict can outlive its field row (for example after alias
    // cleanup). Recover the authoritative row by key before resolving instead
    // of marking the conflict resolved while readiness still shows missing.
