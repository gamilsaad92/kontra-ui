...
function createFakeClient({ failRemove = false } = {}) {
  ...
  return { client: { from: query, storage: { from: storageBucket } }, operations, storageObjects };
}
...
describe('deal room deletion', () => {
  test('removes room-scoped data and verifies storage before deleting the room', async () => {
    ...
    expect(result.preserved).toEqual(expect.arrayContaining(['deal_room_audit_log', 'verified_asset_snapshots']));
    ...
    expect(fake.operations.map(operation => operation.table)).toEqual(expect.arrayContaining([
      'deal_analyses', 'transaction_record_conflicts', 'deal_room_tasks', 'party_submissions',
      'deal_notifications', 'deal_events', 'transaction_record_fields', 'deal_room_invites',
      'deal_room_participants', 'deal_rooms',
    ]));
  });
