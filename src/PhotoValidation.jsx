...
export default function PhotoValidation() {
  ...
  return (
    <div className="bg-white mt-8 rounded-xl shadow-md p-6 w-full max-w-xl">
      ...
      {preview && <img src={preview} alt="Preview" className="mt-4 max-h-64 w-full object-contain rounded border" />}
      <button onClick={handleUpload} className="mt-4 bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded w-full" disabled={loading}>
        {loading ? 'Validating...' : 'Run AI Validation'}
      </button>
      {result && <p className="mt-4 font-medium text-center text-green-700">{result}</p>}
    </div>
  );
}
