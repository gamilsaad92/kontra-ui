...
export default function MessagesScreen() {
  ...
  if (selectedThread) {
    return (
      ...
      >
        <View style={[styles.chatHeader, { borderBottomColor: colors.border, paddingTop: isWeb ? 67 : insets.top }]}>
          <TouchableOpacity onPress={() => setSelectedThread(null)} style={styles.backBtn}>
            <Feather name="arrow-left" size={22} color={colors.foreground} />
          </TouchableOpacity>
          <View style={styles.chatHeaderBody}>
            <Text style={[styles.chatTitle, { color: colors.foreground }]} numberOfLines={1}>{selectedThread.subject}</Text>
            <Text style={[styles.chatSub, { color: colors.mutedForeground }]}>Loan {selectedThread.loan}</Text>
          </View>
        </View>
        ...
        <FlatList
          ...
          data={messages}
          keyExtractor={(m) => m.id}
          contentContainerStyle={[styles.msgList, { paddingBottom: isWeb ? 34 : insets.bottom + 16 }]}
          showsVerticalScrollIndicator={false}
          renderItem={({ item }) => (
            <View style={[styles.msgRow, item.isMe && styles.msgRowMe]}>
              {!item.isMe && (
                <View style={[styles.avatar, { backgroundColor: colors.borrowerLight }]}>
                  <Feather name="user" size={14} color={colors.borrower} />
                </View>
              )}
              <View style={{ maxWidth: "78%" }}>
                <View style={[styles.bubble, item.isMe ? { backgroundColor: colors.borrower } : { backgroundColor: colors.card, borderColor: colors.border, borderWidth: 1 }]}>
                  <Text style={[styles.bubbleText, { color: item.isMe ? "#fff" : colors.foreground }]}>{item.body}</Text>
                </View>
                <Text style={[styles.bubbleTime, { color: colors.mutedForeground, textAlign: item.isMe ? "right" : "left" }]}>{item.time}</Text>
              </View>
            </View>
          )}
        />

        <View style={[styles.inputBar, { borderTopColor: colors.border, backgroundColor: colors.background, paddingBottom: isWeb ? 34 : insets.bottom + 8 }]}>
          <TextInput
            style={[styles.msgInput, { borderColor: colors.border, backgroundColor: colors.card, color: colors.foreground }]}
          ...
          />
          <TouchableOpacity
            style={[styles.sendBtn, { backgroundColor: input.trim() ? colors.borrower : colors.secondary }]}
            onPress={sendMessage}
            disabled={!input.trim()}
            activeOpacity={0.85}
          >
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }) => (
        <TouchableOpacity
          onPress={() => { setSelectedThread(item); Haptics.selectionAsync(); }}
          activeOpacity={0.8}
        >
          <View style={[styles.threadCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            {item.unread && <View style={[styles.unreadDot, { backgroundColor: colors.borrower }]} />}
            <View style={styles.threadBody}>
              <View style={styles.threadTop}>
                <Text style={[styles.threadSubject, { color: colors.foreground, fontFamily: item.unread ? "Inter_600SemiBold" : "Inter_500Medium" }]} numberOfLines={1}>{item.subject}</Text>
                <Text style={[styles.threadDate, { color: colors.mutedForeground }]}>{item.date}</Text>
              </View>
              <Text style={[styles.threadLoan, { color: colors.borrower }]}>Loan {item.loan}</Text>
              <Text style={[styles.threadPreview, { color: colors.mutedForeground }]} numberOfLines={1}>{item.preview}</Text>
            </View>
