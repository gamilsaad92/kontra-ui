    ...
    >
      <View style={styles.statsRow}>
        <StatCard
          ...
          change="+7.2% YTD"
          positive
          accent={colors.investor}
          style={styles.statFlex}
        />
        <StatCard
          label="YTD Distributions"
          value={`$${(totalDist / 1000).toFixed(0)}K`}
          change="+12% vs LY"
          positive
          style={styles.statFlex}
        />
      ...
      <View style={styles.statsRow}>
        <StatCard label="Avg Rate" value="6.21%" style={styles.statFlex} />
        <StatCard label="Avg LTV" value="64%" style={styles.statFlex} />
      </View>
          ...
          {TYPES.map((t) => (
            <TouchableOpacity
              ...
              style={[
                styles.filterChip,
                {
                  backgroundColor: filter === t ? colors.investor : colors.secondary,
                  borderColor: filter === t ? colors.investor : colors.border,
                },
              ]}
              activeOpacity={0.8}
            >
      ...
      <View style={styles.sortRow}>
        ...
        {(["balance", "rate", "ytd"] as const).map((s) => (
          <TouchableOpacity
            key={s}
            onPress={() => setSortBy(s)}
            style={[styles.sortBtn, sortBy === s && { borderBottomWidth: 2, borderBottomColor: colors.investor }]}
          >
            <Text style={[styles.sortText, { color: sortBy === s ? colors.investor : colors.mutedForeground }]}>
              {s === "balance" ? "Balance" : s === "rate" ? "Rate" : "YTD"}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
          ...
          <KontraCard key={h.id} padding={16}>
            <View style={styles.cardTop}>
              <View style={styles.cardLeft}>
                <Text style={[styles.propName, { color: colors.foreground }]}>{h.property}</Text>
                <Text style={[styles.propCity, { color: colors.mutedForeground }]}>{h.city}</Text>
              </View>
              <StatusBadge status={h.status} size="sm" />
            </View>

            <View style={[styles.typeTag, { backgroundColor: colors.investorLight }]}>
              <Text style={[styles.typeText, { color: colors.investor }]}>{h.type}</Text>
            </View>
            ...
            <View style={styles.metrics}>
              {[
                ...
                { label: "LTV", value: `${h.ltv}%` },
                { label: "YTD", value: h.ytd > 0 ? `+${h.ytd}%` : "—" },
              ].map((m) => (
                <View key={m.label}>
            ...
            <View style={[styles.footer, { borderTopColor: colors.border }]}>
              <Text style={[styles.maturity, { color: colors.mutedForeground }]}>Matures {h.maturity}</Text>
              <TouchableOpacity style={[styles.viewBtn, { borderColor: colors.investor }]} activeOpacity={0.8}>
                <Text style={[styles.viewBtnText, { color: colors.investor }]}>Details</Text>
