import { Feather } from "@expo/vector-icons";
  ...
  interface LoanRequest { id: string; type: RequestType; loan: string; property: string; description: string; submittedDate: string; status: "pending" | "review" | "approved" | "rejected"; response?: string; }

  const TYPE_LABELS: Record<RequestType, string> = { payoff: "Payoff Quote", modification: "Loan Modification", extension: "Maturity Extension", advance: "Draw / Advance", other: "General Inquiry" };
  ...
  const REQUEST_TYPES: { id: RequestType; icon: keyof typeof Feather.glyphMap; label: string }[] = [
    { id: "payoff", icon: "dollar-sign", label: "Payoff Quote" }, { id: "modification", icon: "edit", label: "Modification" },
    { id: "extension", icon: "clock", label: "Extension" }, { id: "advance", icon: "download", label: "Draw / Advance" }, { id: "other", icon: "help-circle", label: "Inquiry" },
  ];
  ...
  export default function RequestsScreen() {
    ...
    const handleSubmit = async () => {
      ...
      setRequests(prev => [{ id: `R${Date.now()}`, type: selectedType, loan: "L004", property: "200 Commerce Dr", description: description.trim(), submittedDate: "Apr 16, 2026", status: "pending" }, ...prev]);
      setDescription(""); setSubmitting(false); setShowModal(false);
    ...
    return (
      <>
        <ScrollView style={{ backgroundColor: colors.background }} contentContainerStyle={[s.container, { paddingTop: isWeb ? 83 : insets.top + 16, paddingBottom: isWeb ? 34 : insets.bottom + 80 }]} showsVerticalScrollIndicator={false}>
          <TouchableOpacity style={[s.newBtn, { backgroundColor: colors.borrower }]} onPress={() => { setShowModal(true); Haptics.selectionAsync(); }} activeOpacity={0.85}>
            <Feather name="plus" size={18} color="#fff" /><Text style={s.newBtnText}>New Request</Text>
          </TouchableOpacity>
          <Text style={[s.sectionLabel, { color: colors.mutedForeground }]}>{requests.length} requests</Text>
          <View style={{ gap: 12 }}>
            {requests.map(req => (
              <KontraCard key={req.id} padding={16}>
                <View style={s.reqHeader}><View style={[s.reqTypeBadge, { backgroundColor: colors.borrowerLight }]}><Text style={[s.reqTypeText, { color: colors.borrower }]}>{TYPE_LABELS[req.type]}</Text></View><StatusBadge status={req.status} size="sm" /></View>
                <Text style={[s.reqProp, { color: colors.foreground }]}>{req.property}</Text>
                <Text style={[s.reqLoan, { color: colors.mutedForeground }]}>Loan {req.loan}</Text>
                <Text style={[s.reqDesc, { color: colors.foreground }]}>{req.description}</Text>
                <Text style={[s.reqDate, { color: colors.mutedForeground }]}>Submitted {req.submittedDate}</Text>
                {req.response && <View style={[s.responseBox, { backgroundColor: colors.borrowerLight, borderLeftColor: colors.borrower }]}><Text style={[s.responseLabel, { color: colors.borrower }]}>Servicer Response</Text><Text style={[s.responseText, { color: colors.foreground }]}>{req.response}</Text></View>}
              </KontraCard>
            ))}
          </View>
        </ScrollView>
        <Modal visible={showModal} animationType="slide" presentationStyle="pageSheet">
          <View style={[s.modal, { backgroundColor: colors.background }]}>
            <View style={[s.modalHeader, { borderBottomColor: colors.border }]}>
              <Text style={[s.modalTitle, { color: colors.foreground }]}>New Request</Text>
            ...
            <ScrollView contentContainerStyle={s.modalBody} keyboardShouldPersistTaps="handled">
              <Text style={[s.fieldLabel, { color: colors.mutedForeground }]}>Request Type</Text>
              <View style={s.typeGrid}>
                {REQUEST_TYPES.map(t => (
                  <TouchableOpacity key={t.id} style={[s.typeOption, { backgroundColor: selectedType === t.id ? colors.borrower : colors.secondary, borderColor: selectedType === t.id ? colors.borrower : colors.border }]} onPress={() => { setSelectedType(t.id); Haptics.selectionAsync(); }} activeOpacity={0.8}>
                    <Feather name={t.icon} size={18} color={selectedType === t.id ? "#fff" : colors.mutedForeground} />
                    <Text style={[s.typeOptionText, { color: selectedType === t.id ? "#fff" : colors.foreground }]}>{t.label}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              <Text style={[s.fieldLabel, { color: colors.mutedForeground }]}>Description</Text>
              <TextInput style={[s.textarea, { borderColor: colors.border, backgroundColor: colors.card, color: colors.foreground }]} placeholder="Describe your request in detail..." placeholderTextColor={colors.mutedForeground} value={description} onChangeText={setDescription} multiline numberOfLines={6} textAlignVertical="top" />
              <TouchableOpacity style={[s.submitBtn, { backgroundColor: colors.borrower }, submitting && { opacity: 0.7 }]} onPress={handleSubmit} disabled={submitting || !description.trim()} activeOpacity={0.85}>
