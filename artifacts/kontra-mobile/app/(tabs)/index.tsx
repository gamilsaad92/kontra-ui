...
export default function HomeScreen() {
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={[styles.container, { paddingTop: topPad + 16, paddingBottom: isWeb ? 34 : insets.bottom + 16 }]}
    ...
    >
      <View style={[styles.heroCard, { backgroundColor: accentColor }]}>
        <View style={styles.heroTop}>
          ...
          <View style={[styles.heroBadge, { backgroundColor: "rgba(255,255,255,0.2)" }]}>
            <Text style={styles.heroBadgeText}>{isInvestor ? "Investor" : "Borrower"}</Text>
          </View>
        </View>
        {isInvestor ? (
          <View style={styles.heroStats}>
            <View>
              ...
              <Text style={styles.heroValue}>$12.45M</Text>
            </View>
            <View style={styles.heroSeparator} />
            <View>
              <Text style={styles.heroLabel}>Active Loans</Text>
              <Text style={styles.heroValue}>{loans.length}</Text>
            </View>
            <View style={styles.heroSeparator} />
            <View>
              <Text style={styles.heroLabel}>YTD Return</Text>
              <Text style={styles.heroValue}>+7.2%</Text>
            </View>
          </View>
        ) : (
          <View style={styles.heroStats}>
            <View>
              ...
              <Text style={styles.heroValue}>$9.8M</Text>
            </View>
            <View style={styles.heroSeparator} />
            <View>
              <Text style={styles.heroLabel}>Active Loans</Text>
            ...
            <View>
              ...
              <Text style={styles.heroValue}>May 1</Text>
            </View>
          </View>
        )}
      </View>
      ...
      <View style={styles.quickActions}>
        {isInvestor
          ? [
              { icon: "bar-chart-2" as const, label: "Holdings", route: "/(tabs)/holdings" },
              { icon: "clock" as const, label: "Activity", route: "/(tabs)/activity" },
              { icon: "check-circle" as const, label: "Governance", route: "/(tabs)/governance" },
              { icon: "user" as const, label: "Profile", route: "/(tabs)/profile" },
            ].map((a) => (
              <TouchableOpacity
                key={a.label}
              ...
              >
                ...
                <Text style={[styles.quickLabel, { color: accentColor }]}>{a.label}</Text>
              </TouchableOpacity>
            ))
          : [
              { icon: "file-text" as const, label: "Requests", route: "/(tabs)/requests" },
              { icon: "folder" as const, label: "Documents", route: "/(tabs)/documents" },
              { icon: "message-circle" as const, label: "Messages", route: "/(tabs)/messages" },
              { icon: "user" as const, label: "Profile", route: "/(tabs)/profile" },
            ].map((a) => (
              <TouchableOpacity
                key={a.label}
            ...
            <KontraCard key={loan.id} padding={14}>
              <View style={styles.loanTop}>
                <View style={styles.loanLeft}>
                  <Text style={[styles.loanProp, { color: colors.foreground }]}>{loan.property}</Text>
                  <Text style={[styles.loanType, { color: colors.mutedForeground }]}>{loan.type} · {loan.id}</Text>
                </View>
                <StatusBadge status={loan.status} size="sm" />
              </View>
              <View style={styles.loanBottom}>
                <View>
                  <Text style={[styles.loanMetaLabel, { color: colors.mutedForeground }]}>Balance</Text>
                  <Text style={[styles.loanMetaVal, { color: colors.foreground }]}>
                    ${(loan.balance / 1000000).toFixed(1)}M
                  </Text>
                </View>
                <View>
                  <Text style={[styles.loanMetaLabel, { color: colors.mutedForeground }]}>Rate</Text>
                  <Text style={[styles.loanMetaVal, { color: colors.foreground }]}>{loan.rate}</Text>
                </View>
                <View>
                  <Text style={[styles.loanMetaLabel, { color: colors.mutedForeground }]}>Maturity</Text>
                  <Text style={[styles.loanMetaVal, { color: colors.foreground }]}>{loan.maturity}</Text>
                </View>
                {!isInvestor && "nextPayment" in loan && (
                  <View>
                    <Text style={[styles.loanMetaLabel, { color: colors.mutedForeground }]}>Next Pmt</Text>
                    <Text style={[styles.loanMetaVal, { color: accentColor }]}>{(loan as any).nextPayment}</Text>
                  </View>
                )}
              </View>
            </KontraCard>
          ...
          {activity.map((item) => (
            <KontraCard key={item.id} padding={12}>
              <View style={styles.actRow}>
                <View style={[styles.actDot, { backgroundColor: accentLight }]}>
                  <Feather
                    name={item.type === "Distribution" || item.type === "Payment" ? "dollar-sign" : item.type === "Document" ? "file" : "bell"}
                    size={14}
                    color={accentColor}
                  />
                </View>
                <View style={styles.actBody}>
                  <Text style={[styles.actDesc, { color: colors.foreground }]}>{item.desc}</Text>
                  <Text style={[styles.actDate, { color: colors.mutedForeground }]}>{item.date}</Text>
                </View>
                <Text style={[styles.actAmount, { color: item.amount.startsWith("+") ? colors.success : colors.foreground }]}>
                  {item.amount}
                </Text>
              </View>
            </KontraCard>
...
const styles = StyleSheet.create({
  ...
  heroSeparator: { width: 1, height: 32, backgroundColor: "rgba(255,255,255,0.3)", marginHorizontal: 16 },
