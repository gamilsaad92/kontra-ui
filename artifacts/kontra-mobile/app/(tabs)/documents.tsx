...
const STATUS_COLORS: Record<Doc["status"], { bg: string; text: string }> = {
...
export default function DocumentsScreen() {
  ...
  const handleUpload = async () => {
    ...
    if (Platform.OS === "web") {
      Alert.alert("Upload", "File upload is available on the mobile app.");
      return;
    }
  ...
  const handleCamera = async () => {
    Haptics.selectionAsync();
    if (Platform.OS === "web") {
      Alert.alert("Camera", "Camera capture is available on the mobile app.");
      return;
    }
  ...
  return (
    ...
    >
      {required > 0 && (
        <KontraCard style={[styles.alertCard, { borderLeftColor: colors.destructive }]} padding={14}>
          <View style={styles.alertRow}>
            <Feather name="alert-circle" size={18} color={colors.destructive} />
            <Text style={[styles.alertText, { color: colors.foreground }]}>
              {required} document{required > 1 ? "s" : ""} required from servicer
            </Text>
          </View>
        </KontraCard>
      )}
      ...
      <View style={styles.uploadRow}>
        <TouchableOpacity style={[styles.uploadBtn, { backgroundColor: colors.borrower }]} onPress={handleUpload} activeOpacity={0.85}>
          <Feather name="upload" size={16} color="#fff" />
          <Text style={styles.uploadBtnText}>Upload</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.uploadBtn, { backgroundColor: colors.secondary, borderWidth: 1, borderColor: colors.border }]} onPress={handleCamera} activeOpacity={0.85}>
          <Feather name="camera" size={16} color={colors.foreground} />
          <Text style={[styles.uploadBtnText, { color: colors.foreground }]}>Scan</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
        {CATEGORIES.map((c) => (
          <TouchableOpacity
            key={c}
            onPress={() => setFilter(c)}
            style={[styles.chip, { backgroundColor: filter === c ? colors.borrower : colors.secondary, borderColor: filter === c ? colors.borrower : colors.border }]}
            activeOpacity={0.8}
          >
            <Text style={[styles.chipText, { color: filter === c ? "#fff" : colors.mutedForeground }]}>{c}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
          ...
          return (
            <KontraCard key={doc.id} padding={14}>
              <View style={styles.docRow}>
                <View style={[styles.docIcon, { backgroundColor: colors.borrowerLight }]}>
                  <Feather name={icon} size={18} color={colors.borrower} />
                </View>
                <View style={styles.docBody}>
                  ...
                  <Text style={[styles.docMeta, { color: colors.mutedForeground }]}>
                    {doc.loan} · {doc.category} · {doc.size}
                  </Text>
