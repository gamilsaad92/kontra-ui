...
## Remote tree validation
GitHub's `createTree` response is shallow: nested paths may appear only as a top-level tree entry. Always call `getTree` recursively on the created tree before creating the commit, and verify deployment-critical paths there.

**Why:** A valid-looking tree response can contain only a nested `ui` tree, so checking for `ui/package.json` directly on the create response gives a false failure.

**How to apply:** Validate required paths against the recursive tree response, then create the child commit and advance the live ref with `force:false`.