...
- Vercel install command must use `npm install` (not `npm ci`): `npm ci` consistently crashes on Vercel with "Exit handler never called!" for this project (~1 GB node_modules, 1300+ packages). `npm install --ignore-scripts --no-audit --no-fund` succeeds. Do not revert to `npm ci` — both `--no-audit --no-fund` and `--ignore-scripts` variants of `npm ci` fail the same way.
