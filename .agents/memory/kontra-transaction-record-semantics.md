...
**How to apply:** schema fields should carry separate workflow-required and suggested/expected semantics. Repeated concepts should use one canonical key with display aliases, and extraction/manual entry must normalize aliases before persistence. Dependency rules should make downstream fields non-applicable when a confirmed prerequisite is N/A rather than leaving them outstanding.