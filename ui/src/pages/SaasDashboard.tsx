...
function AuthenticatedDashboard({
  ...
  const renderNavItem = (item: NavItem) => {
    ...
    const state = active
      ? "bg-red-800 text-white shadow-inner shadow-red-900/50"
      : "text-red-200 hover:bg-red-900 hover:text-white focus:bg-red-900 focus:text-white active:bg-red-950/80";
  ...
  return (
    <>
      <div className="flex min-h-screen bg-slate-950 text-red-50">
        <aside className="flex w-64 flex-col border-r border-red-900 bg-red-950 text-red-50">
          <div className="flex items-center gap-3 px-4 py-4 text-sm font-semibold tracking-tight">
            <img src="/logo-dark.png" alt="Kontra logo" className="h-9 w-auto" />
            <div className="leading-tight">
              <div className="text-sm font-semibold text-red-50">Kontra</div>
              <div className="text-xs text-red-200/80">Control Center</div>
            </div>
          </div>
          <nav className="flex-1 space-y-1 overflow-y-auto px-2 pb-4">
            ...
            <div className="pt-4">
             ...
             className="w-full rounded-lg border border-red-800 bg-red-900/40 px-3 py-2 text-sm font-semibold text-red-50 transition hover:bg-red-900 focus:ring-2 focus:ring-red-700" 
                >
                Log Out
              </button>
            </div>
      ...
      <main
          className={`flex-1 overflow-y-auto p-6 ${
            isDashboard
              ? "bg-gradient-to-br from-red-950 via-slate-950 to-red-900 text-red-50"
              : "bg-slate-950 text-red-50"
          }`}
        >
          {!isDashboard && (
            <header className="mb-6 space-y-1">
              <h1 className="text-xl font-semibold tracking-tight text-red-50">{activeLabel}</h1>
