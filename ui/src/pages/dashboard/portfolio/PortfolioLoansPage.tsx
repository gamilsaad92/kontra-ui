import { useState, useMemo } from 'react';
  ...
  const getNum = (d: Record<string, unknown>, ...k: string[]) => {
    const v = pick(d, ...k); return v !== undefined ? parseFloat(String(v)) : undefined;
  };
  const getStr = (d: Record<string, unknown>, ...k: string[]) => {
    const v = pick(d, ...k); return v !== undefined ? String(v) : undefined;
  };
  ...
  // ── Loan create mutation (accepts full form data) ─────────────────────────────
  ...
  export default function PortfolioLoansPage() {
    ...
    const [modal, setModal] = useState<{ mode: 'create' | 'edit'; loan?: CanonicalEntity } | null>(null);
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700 shadow-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500 w-56"
            />
            <select
              value={sortBy}
          ...
          <button
            ...
            className="rounded-lg bg-brand-700 px-4 py-2 text-sm font-medium text-white hover:bg-brand-800 transition"
          >
            + New Loan
          </button>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 hover:border-slate-300 transition">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            {address && <p className="mt-0.5 text-xs text-slate-400 truncate">{address}</p>}
          </div>
          <button
            onClick={onEdit}
      ...
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white py-16 text-center">
        ...
        {!hasLoans && (
          <>
            ...
            <button
              onClick={onCreate}
              className="mt-4 rounded-lg bg-brand-700 px-4 py-2 text-sm font-medium text-white hover:bg-brand-800 transition"
            >
              + New Loan
            </button>
          </>
  ...
  function LoanModal({
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 sm:items-center" onClick={onClose}>
        <div
          className="w-full max-w-xl max-h-[90vh] overflow-y-auto rounded-t-2xl bg-white p-6 shadow-xl sm:rounded-2xl"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field label="Borrower / Sponsor">
                <input value={form.borrower} onChange={set('borrower')}
                  placeholder="Entity or individual name" className={inputCls} />
              </Field>
              ...
              <Field label="Property Type">
                <select value={form.property_type} onChange={set('property_type')} className={inputCls}>
                  {PROP_TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </Field>
              ...
              <Field label="Loan Amount ($)">
                <input type="number" min="0" step="0.01" value={form.loan_amount} onChange={set('loan_amount')}
                  placeholder="e.g. 5000000" className={inputCls} />
              </Field>

              <Field label="LTV (%)">
                <input type="number" min="0" max="200" step="0.01" value={form.ltv} onChange={set('ltv')}
                  placeholder="e.g. 65.0" className={inputCls} />
              </Field>

              <Field label="Interest Rate (%)">
                <input type="number" min="0" step="0.001" value={form.interest_rate} onChange={set('interest_rate')}
                  placeholder="e.g. 6.5" className={inputCls} />
              </Field>

              <Field label="Origination Date">
                <input type="date" value={form.origination_date} onChange={set('origination_date')} className={inputCls} />
              </Field>

              <Field label="Maturity Date">
                <input type="date" value={form.maturity_date} onChange={set('maturity_date')} className={inputCls} />
              </Field>
            </div>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <div className="flex gap-2">
                ...
                <button type="submit" disabled={isPending}
                  className="rounded-lg bg-brand-700 px-4 py-2 text-sm font-medium text-white hover:bg-brand-800 disabled:opacity-50 transition">
  ...
  const inputCls = 'w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-500 placeholder:text-slate-400';
