...
const DISCLOSURE = "Kontra provides software for transaction coordination, document organization, verification support, and digital-asset preparation. Kontra does not provide legal, investment, brokerage, issuance, custody, money-transmission, or regulatory services. Readiness indicators measure deal-room completion and do not constitute legal approval or eligibility for an external process.";

// ── Draft token field disclaimer ─────────────────────────────────────────────
const DRAFT_DISCLAIMER = "Draft information supplied by the deal-room owner. Kontra does not structure, recommend, approve, issue, or sell digital assets.";
...
const SECTIONS = [
  ...
  },
  {
    ...
    description: "Record external-review contacts and supporting status information for KYC, AML, and participant eligibility. Kontra does not perform these reviews.",
    fields: [
      { key: "da_kyc_provider",         label: "External KYC reviewer recorded" },
      { key: "da_aml_provider",         label: "External AML reviewer recorded" },
      { key: "da_accreditation",        label: "External eligibility reviewer recorded" },
      { key: "da_sanctions_screening",  label: "External screening status recorded" },
          ...
          {hasPendingDependencies && (
            <div className="mb-3 rounded-xl bg-amber-50 border border-amber-100 px-3 py-2">
              <p className="text-[10px] text-amber-700 leading-relaxed">
                ...
                requires all sections to be completed before the record is prepared for external review.
              </p>
            </div>
          )}
          {fields.map(f => (
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => {
                ...
                alert("Preparation Package export is not available in this view. Your external reviewer can be sent this deal-room link directly.");
              }}
              disabled={exporting}
