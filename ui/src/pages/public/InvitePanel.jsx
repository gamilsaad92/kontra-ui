...
// ── Custom party — free-form role label + email ─────────────────────────────
function CustomPartyCard({ propertyId, senderName }) {
  ...
  async function handleSend(e) {
    ...
    setStatus('loading');
    setErrMsg('');
    // Derive a URL-safe role key from the label; fall back to "guest"
    const roleKey = label.trim().toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '').slice(0, 32) || 'guest';
    try {
      const res = await fetch(`${API_BASE}/api/public/deal-room/${propertyId}/invite`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ role: roleKey, email: email.trim(), senderName, customLabel: label.trim() }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Failed to send');
      setStatus('sent');
  ...
  function handleCopy() {
    const roleKey = label.trim().toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '').slice(0, 32) || 'guest';
    const url = `${window.location.origin}/deal-room/${propertyId}?role=${roleKey}`;
    navigator.clipboard.writeText(url).then(() => {
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    });
