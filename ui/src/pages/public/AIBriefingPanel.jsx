...
function MorningContent({ briefing, firstName, dealName }) {
...
function AfternoonContent({ standup, briefing, firstName, dealName }) {
    ...
    <>
      <div style={{ padding: '28px 24px 20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          {standup?.narrative ? (
            <p style={{ fontSize: 14, color: '#6b7280', lineHeight: 1.6 }}>{standup.narrative}</p>
          ) : (
            <p style={{ fontSize: 14, color: '#9ca3af' }}>Here's how today went.</p>
          )}
          {briefing && (
            <span style={{
              flexShrink: 0,
      ...
      {risks.length > 0 && (
        <>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 11, fontWeight: 700, color: '#92400e', marginBottom: 6 }}>At risk</p>
            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
              {risks.map((r, i) => (
      ...
      {completedToday.length === 0 && stillBlocked.length === 0 && tomorrowPlan.length === 0 && risks.length === 0 && !standup?.narrative && (
        <div style={{ padding: '0 24px 20px' }}>
          <p style={{ fontSize: 13, color: '#9ca3af' }}>Nothing to report yet today.</p>
        </div>
      )}
    </>
  );
...
export default function AIBriefingPanel({ propertyId, ownerName, dealName, onBriefingLoaded }) {
  const [briefing, setBriefing]   = useState(null);
  const [standup, setStandup]     = useState(null);
  const [briefingErr, setBriefingErr] = useState(null);
  const [standupErr, setStandupErr]   = useState(null);
  ...
  const loadBriefing = useCallback(async () => {
    ...
    try {
      ...
      clearTimeout(timeout);
      setBriefingErr(e.name === 'AbortError' ? 'Timed out' : e.message);
    }
  ...
  const loadStandup = useCallback(async () => {
    ...
    try {
      ...
      clearTimeout(timeout);
      setStandupErr(e.name === 'AbortError' ? 'Timed out' : e.message);
    }
  }, [propertyId]);

  useEffect(() => {
    const t1 = setTimeout(loadBriefing, 400);
    const t2 = setTimeout(loadStandup, 500);
    return () => { clearTimeout(t1); clearTimeout(t2); };
  }, [loadBriefing, loadStandup]);
  ...
  }, [propertyId, loadBriefing, loadStandup]);
  ...
  const isLoading = briefing === null && standup === null && !briefingErr && !standupErr;
  ...
  const bgGradient = morning
    ? 'linear-gradient(160deg, #fdf7f9 0%, #fff 60%)'
    : 'linear-gradient(160deg, #f9fafb 0%, #fff 55%)';
  const borderColor = morning ? '#f0e4e8' : '#e5e7eb';
  ...
  return (
    <div style={{
      ...
      {/* ── Main content — time-of-day adaptive ───────────────────────────── */}
      {morning ? (
        briefing
          ? <MorningContent briefing={briefing} firstName={firstName} dealName={dealName} />
          : <div style={{ padding: '20px 24px' }}>
              <p style={{ fontSize: 13, color: '#9ca3af' }}>
                {briefingErr ? `Couldn't load morning briefing: ${briefingErr}` : 'Loading…'}
              </p>
            </div>
      ) : (
        <AfternoonContent standup={standup} briefing={briefing} firstName={firstName} dealName={dealName} />
      )}

      {/* ── Ask anything — always visible ────────────────────────────────── */}
      <div style={{
        borderTop: `1px solid ${morning ? '#f5ecef' : '#f0f0f0'}`,
        padding: '16px 24px',
        ...
        {(briefingErr || standupErr) && (
          <p style={{ fontSize: 11, color: '#ef4444', marginTop: 8 }}>
            {briefingErr && standupErr ? 'Briefing & standup unavailable' : briefingErr || standupErr}
            {' — '}
            <button onClick={() => { loadBriefing(); loadStandup(); }} style={{
