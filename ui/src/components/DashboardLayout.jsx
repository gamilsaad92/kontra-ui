...
const Placeholder = ({ title }) => (
  <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
    ...
    <p className="mt-2 text-slate-600">
      This section is a placeholder. Wire it to your API or component when ready.
    </p>
  </div>
);
...
export default function DashboardLayout() {
  ...
  // Always land on the main Dashboard ("/") when department changes or first loads
  useEffect(() => {
  ...
  const pages = {
    ...
    // Finance modules you already have:
    ...
    "Guest Chat": () =>
      isFeatureEnabled("hospitality") ? (
        ...
        <Placeholder title="Guest Chat" />
      ),
  };

  // Auto-generate placeholder routes for items that don’t have a page yet
  const ensurePage = (label) => pages[label] || (() => <Placeholder title={label} />);

  const routes = navItems.map((item) => (
    <Route key={item.label} path={toPath(item.label)} element={ensurePage(item.label)()} />
  ));

  // Also map any extra pages you already use that aren’t in the sidebar
  routes.push(<Route key="root" path="/" element={<DashboardHome navigateTo={navigateTo} />} />);
  ...
  return (
    <div className="flex min-h-screen flex-col lg:flex-row bg-slate-100 text-slate-900">
      ...
      <aside
        ...
        } w-full bg-slate-950 text-slate-100 flex flex-col transition-all`}
