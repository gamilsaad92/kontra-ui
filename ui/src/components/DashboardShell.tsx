...
const topTabs = ["Lender/Servicer","Investor","Borrower","Contractor","Admin"] as const;
const lenderSubTabs = [
...
export default function DashboardShell() {
  ...
  const [activeTop, setActiveTop] = useState<(typeof topTabs)[number]>("Lender/Servicer");
  ...
  const subTabs = useMemo(() => {
    switch (activeTop) {
      case "Lender/Servicer": return lenderSubTabs;
      default: return [];
    }
  }, [activeTop]);

  return (
    <div className="min-h-screen bg-gradient-to-b from-[rgb(8,12,20)] to-[rgb(6,8,14)] text-slate-100 dark:text-slate-100">
      {/* Top bar */}
      <header className="flex items-center justify-between px-5 py-3 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <nav className="flex items-center gap-2">
            {topTabs.map(t => (
              <Link
                key={t}
                to={t==="Lender/Servicer" ? "/lender/portfolio" : `/${t.toLowerCase().split("/")[0]}`}
                className={`px-3 py-1.5 rounded-full text-sm ${t===activeTop ? "bg-white/10" : "hover:bg-white/5"}`}
              >
                {t}
              </Link>
            ))}
        ...
        <div className="flex items-center gap-2">
          <button className="p-2 rounded-full hover:bg-white/10"><Search size={18}/></button>
          <button className="p-2 rounded-full hover:bg-white/10"><Settings size={18}/></button>
        ...
        <nav className="flex gap-5 text-sm overflow-x-auto">
          {subTabs.map(s => (
            <NavLink key={s.to} to={s.to} className={({isActive}) =>
              `py-3 border-b-2 -mb-px ${isActive ? "border-indigo-500" : "border-transparent hover:border-white/20"}`
            }>
              {s.label}
            </NavLink>
          ))}
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur p-4">
            ...
            <ul className="space-y-2 text-sm">
              <li className="flex items-start gap-2"><span className="mt-1 h-2 w-2 bg-green-500 rounded-full"/><div><b>Report generated</b><div className="text-xs opacity-60">2 minutes ago</div></div></li>
              <li className="flex items-start gap-2"><span className="mt-1 h-2 w-2 bg-yellow-400 rounded-full"/><div><b>Loan LN-1021 at risk</b><div className="text-xs opacity-60">1 hour ago</div></div></li>
            </ul>
          </div>
