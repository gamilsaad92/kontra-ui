...
function isFolderEntry(entry) {
  // Supabase Storage represents folders with a null id and null metadata.
...
/**
 * Delete the mutable/operational portion of a Deal Room.
 *
 * Storage is cleaned and re-listed before any database rows are removed.
 * Database operations are intentionally ordered from children to parent and
 * are safe to retry after a partial database failure. Immutable audit logs and
 * Verified Asset snapshots are deliberately not touched.
 */
async function deleteDealRoomData(propertyId, client = supabase) {
  ...
  // The active upload path and generated preparation PDFs use this private
  // bucket. Re-listing after remove is what prevents false success.
  // Remove metadata-referenced paths first in case an older artifact used a
  // path outside the normal room prefix. Prefix cleanup below then catches
  // every remaining object, including unreferenced uploads.
  const storage = {
  ...
  // v2 has a separate table and bucket. Only inspect that bucket when the v2
  // document table exists for this deployment.
  if (!v2Documents.skipped) {
    storage[V2_DOCUMENT_BUCKET] = await removeStoragePrefix(client, V2_DOCUMENT_BUCKET, propertyId);
  }

  const documentResult = await deleteRows(
    client,
    'deal_room_documents',
    q => q.eq('property_id', propertyId),
  );
  ...
  const pdfResult = await deleteRows(
    client,
    'digital_asset_preparation_pdf_artifacts',
    q => q.eq('property_id', propertyId),
  );
  ...
  const revisionResult = await deleteRows(
    client,
    'digital_asset_preparation_package_revisions',
    q => q.eq('property_id', propertyId),
  );
  markResult('digital_asset_preparation_package_revisions', revisionResult);

  const preparationResult = await deleteRows(
    client,
    'digital_asset_preparation_packages',
    q => q.eq('property_id', propertyId),
  );
  ...
  // Legacy invite sessions reference legacy invites.
  const legacySessionResult = await deleteRowsByIds(
    client,
    'deal_room_access_sessions',
    'invite_id',
    legacyInvites.rows.map(row => row.id).filter(Boolean),
  );
  ...
  const legacyInviteResult = await deleteRows(
    client,
    'deal_room_invites',
    q => q.eq('property_id', propertyId),
  );
  ...
  // v2 roles and OTP requests reference v2 participants/invites and therefore
  // must be cleared before those parent rows.
  const participantRoleResult = await deleteRowsByIds(
    client,
    'deal_room_participant_roles',
    'participant_id',
    v2Participants.rows.map(row => row.id).filter(Boolean),
  );
  ...
  const otpResult = await deleteRowsByIds(
    client,
    'deal_room_otp_requests',
    'invite_id',
    v2Invites.rows.map(row => row.id).filter(Boolean),
  );
  ...
  const participantResult = await deleteRows(
    client,
    'deal_room_participants',
    q => q.eq('room_id', propertyId),
  );
  markResult('deal_room_participants', participantResult);

  const v2InviteResult = await deleteRows(
    client,
    'deal_room_invites_v2',
    q => q.eq('room_id', propertyId),
  );
  markResult('deal_room_invites_v2', v2InviteResult);

  // Keep this last. If any preceding operation fails, the room remains and
  // the same owner request can retry the incomplete cleanup.
  const roomResult = await deleteRows(
    client,
    'deal_rooms',
    q => q.eq('property_id', propertyId),
    { optional: false },
