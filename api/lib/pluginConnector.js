...
const { TRANSACTION_NOTIFICATION_ADDRESS } = require('./emailConfig');
...
const BUILT_IN_CONNECTORS = [
...

  {
    ...
    async execute(action, payload, creds) {
      ...
      const fetch = (await import('node-fetch')).default;
      const body  = { personalizations: [{ to: [{ email: payload.to }] }], from: { email: payload.from || TRANSACTION_NOTIFICATION_ADDRESS }, subject: payload.subject || 'Kontra Notification', content: [{ type: 'text/plain', value: payload.text || payload.body || '' }] };
