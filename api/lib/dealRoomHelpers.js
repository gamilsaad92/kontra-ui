...
async function sealClosingRecord(propertyId) {
  try {
    ...
    // ── Fetch a previously stored legacy VAP, if one exists ───────────────────
    // If a legacy VAP was already persisted, read it here to embed a permanent
    // copy in the closing record snapshot. Lifecycle advancement never creates
    // a package; new preparation packages are explicitly snapshot-bound.
